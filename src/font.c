...
void Font_CDR_DrawCol(struct FontData *this, const char *text, fixed_t x, fixed_t y, FontAlign align, u8 r, u8 g, u8 b, const char *menu)
{
	...
	while ((c = *text++) != '\0')
	{
		...
		//Draw character
		RECT src = {font_cdrmap[c].charX, 69 + font_cdrmap[c].charY, font_cdrmap[c].charW, font_cdrmap[c].charL};
		RECT_FIXED dst = {x << FIXED_SHIFT, y << FIXED_SHIFT, src.w << FIXED_SHIFT, src.h << FIXED_SHIFT};
...
void FontData_Load(FontData *this, Font font)
{
	...
	switch (font)
	{
		...
		case Font_Arial:
			//Load texture and set functions
			Gfx_LoadTex(&this->tex, IO_Read("\\FONT\\ARIACDR.TIM;1"), GFX_LOADTEX_FREE);
